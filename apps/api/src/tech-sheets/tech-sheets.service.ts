...
export class TechSheetsService {
  ...
  async getPdfHtml(id: number, userId: number): Promise<string> {
    ...
    const ingredients: Array<{ name: string; quantity: string; unit?: string; unitPrice?: number }> =
    ...
    const difficultyLabel = { facile: 'Facile', moyen: 'Moyen', difficile: 'Difficile' }[sheet.difficulty ?? ''] ?? ''
    ...
    const ingredientRows = ingredients
      .map(
        ...
        <tr>
          ...
          <td>${ing.quantity}</td>
          <td>${ing.unit ?? ''}</td>
          <td>${ing.unitPrice != null ? `${Number(ing.unitPrice).toFixed(2)} €` : ''}</td>
        </tr>`,
      )
      .join('')
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .meta { display: flex; gap: 24px; padding: 12px 16px; background: #f5f5f4; border-radius: 8px; margin-bottom: 20px; }
...
<body>
  ...
  <h2>Ingrédients</h2>
  <table>
    <thead><tr><th>Ingrédient</th><th>Quantité</th><th>Unité</th><th>Prix unit.</th></tr></thead>
  ...
  ${sheet.presentation ? `
  <h2>Présentation / Dressage</h2>
  <div class="note-box">${sheet.presentation}</div>` : ''}

  ${sheet.tips ? `
  <h2>Conseils du chef</h2>
  <div class="note-box">${sheet.tips}</div>` : ''}

  <div class="footer">
    <span>Chef IA — Fiche technique</span>
    <span>${date}</span>
  </div>
